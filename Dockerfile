...
USER root
RUN $APT_CMD update \
 && $APT_CMD install -y curl wget unzip zenity q4wine winehq-staging wine-staging wine-staging-amd64 wine-staging-i386:i386 libpcsclite1 pcscd pcsc-tools usbutils \
 && apt-get clean \
